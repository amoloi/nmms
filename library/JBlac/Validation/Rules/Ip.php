...
class JBlac_Validation_Rules_Ip extends JBlac_Validation_Rules_AbstractRule
{
    public $ipOptions;
    ...
    protected function parseRange($input)
    {
        ...
        if (!$this->verifyAddress($range['min'])) {
            throw new JBlac_Validation_Exceptions_ComponentException('Invalid network range');
        }
    ...
    protected function parseRangeUsingCidr($input, &$range)
    {
        ...
        if ($isAddressMask || $input[1] < 8 || $input[1] > 30) {
            throw new JBlac_Validation_Exceptions_ComponentException('Invalid network mask');
        }
